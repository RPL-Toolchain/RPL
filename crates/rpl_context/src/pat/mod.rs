...
impl<'pcx> Pattern<'pcx> {
    ...
    fn add_item(
        ...
        match &***item {
            ...
            Choice4::_1(rust_struct) => self.add_struct(pat_name, WithPath::new(p, rust_struct), symbol_table),
            Choice4::_2(rust_enum) => self.add_enum(pat_name, WithPath::new(p, rust_enum), symbol_table),
            Choice4::_3(rust_impl) => self.add_impl(pat_name, WithPath::new(p, rust_impl), meta, symbol_table),
        }
    }
    ...
    fn add_impl(
        ...
        let fns = fns
            ...
            .map(|rust_fn| {
                ...
                let fn_def = Fn::from(WithPath::new(p, rust_fn), self.pcx, fn_sym_tab, Arc::clone(&meta));
                (fn_name, fn_def)
            })
            .collect();
        let impl_pat = Impl {
